...
static bool ble_smtcn_gatts_app_response = pdFALSE;  //true: APP wants to rsp GATT r/w by itself.
static int ble_smtcn_serv_hdl = -1;
#endif /* #ifdef __BT_GATTS_EH__ */
...
void ble_smtcn_setup_wifi_connection(void)
{
    ...
    result = wifi_config_set_ssid(port, p_smtcn->ssidrx, p_smtcn->ssidrx_len);
    LOG_I(BLE_SMTCN, "[BLE_SMTCN]set ssid = %d (%s, %d)\n", result, p_smtcn->ssidrx, p_smtcn->ssidrx_len);
    if (result < 0) {
        return;
    }
...
static uint32_t ble_smtcn_client_config_callback(const uint8_t rw, uint16_t handle, void *data, uint16_t size, uint16_t offset)
{
    ...
    if (rw == BT_GATTS_CALLBACK_WRITE) {
        ...
        if (p_smtcn->indicate_enable == 0x0002 && p_smtcn->dtp_timer) {
            //send indication
...
#ifdef __BT_GATTS_EH__
    if (ble_smtcn_gatts_app_response) {
        ble_smtcn_serv_hdl = bt_gatts_get_service_handle(&bt_if_ble_smtcn_service, (bt_gatts_service_rec_t *)&bt_if_dtp_client_config);
...
#ifdef __BT_GATTS_EH__
void ble_smtcn_set_server_response_flag(bool send)
{
    ble_smtcn_gatts_app_response = send;
}

void ble_smtcn_send_write_response(bt_status_t st)
{
    bt_gatts_send_write_response(p_smtcn->conn_handle, ble_smtcn_serv_hdl, st);
}

void ble_smtcn_send_read_response(uint8_t *data, uint16_t size, uint16_t offset, bt_status_t st)
{
    bt_gatts_send_read_response(p_smtcn->conn_handle, ble_smtcn_serv_hdl, data, size, offset, st);
}
#endif /* #ifdef __BT_GATTS_EH__ */
