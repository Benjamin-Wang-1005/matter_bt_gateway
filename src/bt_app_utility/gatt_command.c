...
const bt_gatts_service_t **default_bqb_get_gatt_server(void)
{
    return NULL;
}
...
{
    LOG_I(common, "%s 0x%04x 0x%02x", __func__, handle, flag);
#ifdef MTK_BLE_SMTCN_ENABLE
    ble_smtcn_charc_execute_write_callback(handle);
#endif
    return BT_STATUS_SUCCESS;
}
...
static void dump_uuid128(bt_uuid_t *uuid128)
{
    ...
    for (j = 15, i = 0; j >= 0; j--, i += 2) {
        cn = snprintf((char *)(buf + i), 3, "%02x", uuid128->uuid[j]);
        if (cn < 0 || cn > 3)
            return;
    }
...
static uint8_t *get_uuid128(bt_uuid_t *uuid128)
{
    ...
    for (j = 15, i = 0; j >= 0; j--, i += 2) {
        cn = snprintf((char *)(tmp_uuid_buf + i), 3, "%02x", uuid128->uuid[j]);
        if (cn < 0 || cn > 3)
            return NULL;
    }
    return tmp_uuid_buf;
...
static void dump_char_properties(uint8_t prop)
{
    ...
    if (prop & 0x01) {
        ...
        cn = snprintf(ptr, strlen(prop_str) + 1, "%s", prop_str);
        if (cn < 0 || cn > (int)strlen(prop_str) + 1)
            return;
        ptr += strlen(ptr);
    }
    if (prop & 0x02) {
...
static void gatt_free_all_service(void)
{
    gatt_service_node *service_node;
...
static void print_all_service(list_entry *service_list)
{
    ...
    gatt_service *srv;
    if (!service_list || is_list_empty(service_list))
        return;
...
static void real_begin_discover_next_charc(void)
{
...
static void real_begin_discover_next_include_service(void)
{
    cur_srv_index++;
...
static void begin_discover_desc(void)
{
...
static void discover_next_desc(uint8_t skip, uint16_t cur_handle)
{
    ...
    if (!char_node) {
        ...
        if ((list_entry *)char_node->node.next != &cur_srv_node->char_list) {
            ...
            start_handle = char_node->characteristic.value_handle + 1;
            if (start_handle < cur_handle)
                start_handle = cur_handle;
            end_handle = cur_srv_node->service.end_handle;
            if (0 == skip) {
                ble_gattc_discover_descriptors(conn_id, start_handle, end_handle);
            } else {
...
static bt_status_t bt_gattc_write_data_test(void)
{
...
static bt_status_t ble_gattc_write_data_test(void)
{
    uint8_t buf[260] = { 0 };
...
static void show_throughput(void)
{
    if (test_started) {
        cur_tick = xTaskGetTickCount();
        if (cur_tick > test_finish_time) {
...
bt_status_t bt_cmd_gattc_io_callback(void *input, void *output)
{
    ...
    if (UT_APP_CMP("gatt discover_all")) {
        ...
        if (ulcn > 0xFFFF) {
            LOG_E(common, "gatt discover_all, handle should be 0x0000 ~ 0xFFFF");
            return BT_STATUS_FAIL;
        }
        conn_id = (uint16_t)ulcn;
        ...
        ulcn = strtoul(attribute_handle, NULL, 16);
        if (ulcn > 0xFFFF) {
            LOG_E(common, "gatt read, attribute_handle should be 0x0000 ~ 0xFFFF");
            return BT_STATUS_FAIL;
        }
        req.attribute_handle = (uint16_t)ulcn;

        ulcn = strtoul(handle, NULL, 16);
        if (ulcn > 0xFFFF) {
            LOG_E(common, "gatt read, conn_id should be 0x0000 ~ 0xFFFF");
            return BT_STATUS_FAIL;
        }
        conn_id = (uint16_t)ulcn;
        ...
        if (len < 1) {
            LOG_E(common, "gatt write, attribute_value length < 1");
            return BT_STATUS_FAIL;
        }
        ...
        ulcn = strtoul(handle, NULL, 16);
        if (ulcn > 0xFFFF) {
            LOG_E(common, "gatt write, handle should be 0x0000 ~ 0xFFFF");
            return BT_STATUS_FAIL;
        }
        conn_id = (uint16_t)ulcn;
        ...
        ulcn = strtoul(attribute_handle, NULL, 16);
        if (ulcn > 0xFFFF) {
            LOG_E(common, "gatt write, attribute_handle should be 0x0000 ~ 0xFFFF");
            return BT_STATUS_FAIL;
        }
        req.att_req->attribute_handle = (uint16_t)ulcn;
        ...
        ulcn = strtoul(handle, NULL, 16);
        if (ulcn > 0xFFFF) {
            LOG_E(common, "gatt send_data_test, test_conn_id should be 0x0000 ~ 0xFFFF");
            return BT_STATUS_FAIL;
        }
        test_conn_id = (uint16_t)ulcn;

        ulcn = strtoul(duration, NULL, 10) * 1000 / portTICK_PERIOD_MS;
        if (ulcn > 0xFFFF) {
            LOG_E(common, "gatt send_data_test, time_duration should be 0x0000 ~ 0xFFFF");
            return BT_STATUS_FAIL;
        }
        time_duration = (uint16_t)ulcn;
        ...
        ulcn = strtoul(handle, NULL, 16);
        if (ulcn > 0xFFFF) {
            LOG_E(common, "gatt mtu, test_conn_id should be 0x0000 ~ 0xFFFF");
            return BT_STATUS_FAIL;
        }
        test_conn_id = (uint16_t)ulcn;

        ulcn = strtoul(cmtu, NULL, 10);
        if (ulcn > 0xFFFF) {
            LOG_E(common, "gatt mtu, cmtu should be 0x0000 ~ 0xFFFF");
            return BT_STATUS_FAIL;
        }
        BT_GATTC_NEW_EXCHANGE_MTU_REQ(req, (uint16_t)ulcn);
