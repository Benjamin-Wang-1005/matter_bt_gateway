...
BT_GATTS_NEW_CHARC_VALUE_UINT32_WRITABLE(gatt_service_changed, CLI_BT_SIG_UUID_SERVICE_CHANGED,
                                         0x2, 0x050F0001); //characteristic value = 0x[End hdl(2bytes) | start hdl(2bytes)] = 0x050F0001
...
void gatt_service_change_notify(bt_handle_t connection_handle)
{
    uint8_t buf[64] = {0};
    ...
    bas_noti_rsp->att_req.handle = GATTS_SERVICE_CHANGE_HANDLE;
...
{
    gatt_service_change_notify(connection_handle);
}
...
#endif // end -- #ifdef MTK_GATT_JITTER_TEST
