...
static uint8_t ut_app_reset_flash_flag = false;
...
bt_status_t bt_app_io_callback(void *input, void *output)
{
    ...
    */
        ...
        if (bt_get_init_mode() == user_mode) {
            BT_LOGI("APP", "no need to switch");
        } else {
            if (bt_save_init_mode((uint8_t)user_mode))
                bt_reboot_system();
            else
                BT_LOGE("APP", "boot mode save fail!");
        }
    }
    ...
    */
    ...
    /* Usage:   bond <handle in hex>
       Example: bond 0200 */
        ...
        bt_bd_addr_ptr_t p_local_public_addr = NULL;
#ifndef MTK_BLE_DM_SUPPORT
        p_local_public_addr = (bt_bd_addr_ptr_t)&g_local_public_addr;
#else /* #ifndef MTK_BLE_DM_SUPPORT */
        p_local_public_addr = (bt_bd_addr_ptr_t)bt_device_manager_get_local_address();
#endif /* #ifndef MTK_BLE_DM_SUPPORT */
        BT_LOGI("APP", "local_public_addr [%02X:%02X:%02X:%02X:%02X:%02X]",
                p_local_public_addr[5], p_local_public_addr[4], p_local_public_addr[3],
                p_local_public_addr[2], p_local_public_addr[1], p_local_public_addr[0]);
    }
#ifdef MTK_MT7933_BT_ENABLE
...
static void bt_ut_gap_le_scan_list_add(void *data, bt_ut_gap_le_scan_list_t *list)
{
    ...
    if (p->event_type != 0 && report->event_type != 0 && p->name[0] != '\0')
        return;

    while (count < report->data_length) {
        /*
         * Report data format : | Data length (1byte) | type (1byte) | Data (Data length bytes) |
         * So if remain length less than 2, skip it
         */
        if (count >= BT_HCI_LE_ADVERTISING_DATA_LENGTH_MAXIMUM - 2) {
            BT_LOGW("APP", "larger than report len (%u)", count);
            break;
        ...
        ad_data_len = report->data[count];
        /* Shift for length field */
        count += 1;
        /* Error handling for remain data not enough. */
        if (ad_data_len > BT_HCI_LE_ADVERTISING_DATA_LENGTH_MAXIMUM - count ||
            ad_data_len > report->data_length - count) {
            BT_LOGI("APP", "AD Data Len Error %d, data_length=%d, count=%d",
                    ad_data_len, report->data_length, count);
            break;
        }
        ad_data_type = report->data[count];
        ...
        /* If the data is name, copy it! */
        if (ad_data_type == BT_GAP_LE_AD_TYPE_NAME_COMPLETE) {
            uint8_t cp_len = ad_data_len > BT_UT_GAP_LE_SCAN_LIST_MAX_NAME_LEN ? BT_UT_GAP_LE_SCAN_LIST_MAX_NAME_LEN: ad_data_len;

            p->name_exist = true;
            /*
             * If cp_len equal to ad_data_len, we need to minus 1 to bypass type field.
             * If cp_len equal to BT_UT_GAP_LE_SCAN_LIST_MAX_NAME_LEN, we need to minus 1 to preserve end character.
             * So we minus 1 after confirmed the value of cp_len.
             */
            cp_len--;
            memcpy(p->name, report->data + count, cp_len);
            p->name[cp_len] = '\0';
            break;
        }
...
bt_status_t bt_ut_app_event_callback(bt_msg_type_t msg, bt_status_t status, void *buff)
{
    if (msg != BT_MEMORY_FREE_GARBAGE_IND &&
        ...
        && (msg & BT_LE_AUDIO_MODULE_MASK) != BT_LE_AUDIO_MODULE_MCP
#endif /* #ifdef BT_LE_AUDIO_ENABLE */
       ) {
        BT_COLOR_SET(BT_COLOR_GREEN);
        BT_LOGI("APP", "evt_cb: status(0x%04x), msg = 0x%x", status, msg);
        BT_COLOR_SET(BT_COLOR_WHITE);
    }
...
#if defined(__BT_HB_DUO__) && !defined(__MTK_BT_SINK_SUPPORT__)
...
#endif /* #if defined(__BT_HB_DUO__) && !defined(__MTK_BT_SINK_SUPPORT__) */
    }
