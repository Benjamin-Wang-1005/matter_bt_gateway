...
static void bt_generate_address(bt_bd_addr_t addr, const char *item_name)
{
    ...
    nvdm_status_t status = nvdm_read_data_item("BT", item_name, buffer, &size);
    if (NVDM_STATUS_OK == status) {
        ...
        LOG_D(common, "[BT]Read %s from NVDM [%02X:%02X:%02X:%02X:%02X:%02X]\n", item_name,
              addr[5], addr[4], addr[3], addr[2], addr[1], addr[0]);
        return;
    }
...
#ifdef MTK_NVDM_ENABLE
    nvdm_status_t status = nvdm_read_data_item("BT_MODE", "initmode", &init_mode, &size);
...
bool bt_save_init_mode(uint8_t init_mode)
{
    ...
    BT_LOGI("APP", "NVDM did not enabled!");
    g_bt_boot_init_mode = init_mode;
    return false;
#endif /* #ifdef MTK_NVDM_ENABLE */
}
...
void bt_create_task(void)
{
    ...
    * (((uint8_t *)g_local_random_addr) + 5) |= 0xC0;
    ...
    //int ret =
    snprintf(dev_name, BT_GAP_MAX_DEVICE_NAME_LENGTH, "HB Duo dev %02x-%02x",
