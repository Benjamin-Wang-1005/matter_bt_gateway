...
#define COMMAND_PARAM_NUMBER     50
#define COMMAND_BUF              300
static int wifi_console = 0;
static uint8_t data_start = 0;
static uint8_t len = 0;
static char *param[COMMAND_PARAM_NUMBER];
static char cmd_buf[COMMAND_BUF] = {0};
    ...
    while (1) {
        ...
        while ((rlen = lwip_read(wifi_console, srv_buf, sizeof(srv_buf) - 1)) != 0) {
            ...
            if (rev_cmd) {
                ...
                data_start = 0;
                len = 0;
                os_memset(cmd_buf, 0, COMMAND_BUF);
                os_memset(param, 0, COMMAND_PARAM_NUMBER);
