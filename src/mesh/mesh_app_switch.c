#ifndef __MESH_APP_SWITCH_C__
#define __MESH_APP_SWITCH_C__
/* Copyright Statement:
 *
...
#include "bt_mesh_model_scene_client.h"
#include "bt_mesh_model_time_client.h"
#include "bt_mesh_model_scheduler_client.h"

#ifdef APP_BT_MESH_OTA_ENABLE
#include "bt_mesh_ota_distributor.h"
#include "bt_mesh_ota_initiator.h"
#endif /* #ifdef APP_BT_MESH_OTA_ENABLE */
...
#define SUPPORT_MESH_FILTER
//#define MESH_PROPRIETARY_SRV
//#define MESH_APP_PERFORMANCE_TEST
...
#define UNPROVISIONED_DEV_REC_MAX    5
...
#endif /* #ifdef APP_BT_MESH_OTA_ENABLE */
#ifdef MESH_APP_PERFORMANCE_TEST
...
#endif /* #ifdef MESH_APP_PERFORMANCE_TEST */

#ifdef MESH_PROPRIETARY_SRV
#define MESH_ADV_TIMER_ID (mesh_adv_timer)
#endif /* #ifdef MESH_PROPRIETARY_SRV */
...
#define MESH_APP_TASK_NAME "mesh_app"
#define MESH_APP_TASK_STACKSIZE 1024*2
#define MESH_APP_TASK_PRIO TASK_PRIORITY_NORMAL
...
void _get_remote_info_timeout_callback_post_handler(TimerHandle_t handle);
...
void mesh_app_post_prov_event(uint8_t *prov_data);
...
#endif /* #ifdef MESH_APP_PERFORMANCE_TEST */
#ifdef MESH_PROPRIETARY_SRV
static TimerHandle_t mesh_adv_timer = NULL; /**< Timer handler. */
#endif /* #ifdef MESH_PROPRIETARY_SRV */
...
static uint8_t g_def_netkey[16] = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF};
static uint16_t g_def_netkeyIdx = 0x00;
...
// element index
uint16_t g_config_client_element_index = BT_MESH_MODEL_SIG_MODEL_ID_CONFIGURATION_CLIENT;
...
static uint16_t g_model_handle_scene = 0;
static uint16_t g_model_handle_time = 0;
static uint16_t g_model_handle_scheduler = 0;
...
static bt_addr_t g_target_addr;
static bt_handle_t g_slave_handle = 0xFFFF;

#ifdef PROVISIONER_ENABLE
static bool g_provision_by_gatt = false;
#endif /* #ifdef PROVISIONER_ENABLE */


#define MESH_PROPIETARY_SERVICE_START_HANDLE            0x0301
#define MESH_PROPIETARY_SERVICE_END_HANDLE              0x0303
#define MESH_PROPIETARY_SERVICE_CHAR_VALUE_HANDLE       MESH_PROPIETARY_SERVICE_START_HANDLE + 2

#define BT_GATT_UUID16_MESH_PROPIETARY_SERVICE                                  0x1900      /**< Mesh Propietary service. */

#define BT_SIG_UUID16_MESH_DEVICE                                               0x2AFF      /**< Mesh Device. */

const bt_uuid_t CLI_BT_SIG_UUID_MESH_DEVICE =
    BT_UUID_INIT_WITH_UUID16(BT_SIG_UUID16_MESH_DEVICE);

#ifdef MESH_PROPRIETARY_SRV
static uint32_t bt_mesh_propietary_callback(const uint8_t rw, uint16_t handle, void *data, uint16_t size, uint16_t offset);

/* Mesh Propietary service*/
BT_GATTS_NEW_PRIMARY_SERVICE_16(bt_if_mesh_propietary_primary_service, BT_GATT_UUID16_MESH_PROPIETARY_SERVICE);
BT_GATTS_NEW_CHARC_16_WRITABLE(bt_if_mesh_char4_propietary_dev, BT_GATT_CHARC_PROP_READ | BT_GATT_CHARC_PROP_WRITE | BT_GATT_CHARC_PROP_WRITE_WITHOUT_RSP, MESH_PROPIETARY_SERVICE_CHAR_VALUE_HANDLE, BT_SIG_UUID16_MESH_DEVICE);
BT_GATTS_NEW_CHARC_VALUE_CALLBACK(bt_if_mesh_propietary_dev, CLI_BT_SIG_UUID_MESH_DEVICE,
                                  BT_GATTS_REC_PERM_READABLE | BT_GATTS_REC_PERM_WRITABLE, bt_mesh_propietary_callback);

static const bt_gatts_service_rec_t *bt_if_mesh_propietary_service_rec[] = {
    (const bt_gatts_service_rec_t *) &bt_if_mesh_propietary_primary_service,
    (const bt_gatts_service_rec_t *) &bt_if_mesh_char4_propietary_dev,
    (const bt_gatts_service_rec_t *) &bt_if_mesh_propietary_dev,
};

const bt_gatts_service_t bt_if_mesh_propietary_service = {
    .starting_handle = 0x0301,
    .ending_handle = 0x0303,
    .required_encryption_key_size = 0,
    .records = bt_if_mesh_propietary_service_rec
};
#endif /* #ifdef MESH_PROPRIETARY_SRV */
...
static uint32_t bt_mesh_propietary_callback(const uint8_t rw, uint16_t handle, void *data, uint16_t size, uint16_t offset)
{
    switch (rw) {
        ...
        case BT_GATTS_CALLBACK_WRITE: {
                if (size != 38) {
                    LOG_E(mesh_app, "Data size should be 38 bytes");
                    return size;
                }

                mesh_app_post_prov_event(data);
                return size;
            }
        default:
...
static void _custom_adv_timeout_handler(TimerHandle_t handle)
{
    uint8_t ad[19] = {
        0x02, 0x1, 0x05,
        0xA8, 0x01, 0x0A, 0x03
    ...
    bt_hci_cmd_le_set_advertising_parameters_t parm = {
    ...
    bt_mesh_bearer_adv_send_by_time(sizeof(ad), ad, 40, &parm);

    bt_mesh_app_util_start_timer(handle, 1000);
}
#endif /* #if MESH_PROPRIETARY_SRV */
static void _node_reset_retry_callback_post_handler(TimerHandle_t handle)
{
...
{
    mesh_app_post_timeout_event(handle);
}

static void _configure_callback_post_handler(TimerHandle_t handle)
{
...
static void _switch_led_timer_cb_post_handler(TimerHandle_t handle)
{
    switch (ledPhase++) {
        case PHASE_0: {
                                      ...
                                      const bt_mesh_access_message_rx_t *msg, const bt_mesh_scene_client_evt_t *event)
{
    ...
    switch (event->evt_id) {
        case BT_MESH_SCENE_CLIENT_EVT_STATUS: {
                ...
                LOG_I(mesh_app, "StatusCode   : %02x\n", evt->status_code);
                LOG_I(mesh_app, "CurrentScene : %04x\n", evt->current_scene);
                if (msg->length > 3) {
                    LOG_I(mesh_app, "TargetScene  : %04x\n", evt->target_scene);
                    LOG_I(mesh_app, "RemainingTime: %02x\n", evt->remaining_time);
                }
                break;
            }
        case BT_MESH_SCENE_CLIENT_EVT_REGISTER_STATUS: {
                ...
                LOG_I(mesh_app, "StatusCode   : %02x\n", evt->status_code);
                LOG_I(mesh_app, "CurrentScene : %04x\n", evt->current_scene);
                LOG_I(mesh_app, "Scenes  : %d {", evt->scene_list_size);
                if (evt->scene_list != NULL && evt->scene_list_size > 0) {
                    uint16_t count = 0;
                    while (count < evt->scene_list_size) {
                        LOG_I(mesh_app, "%04x", evt->scene_list[count]);
                        count++;
                    }
                }
                LOG_I(mesh_app, "}");
...
static void _time_client_msg_handler(uint16_t model_handle,
                                     const bt_mesh_access_message_rx_t *msg, const bt_mesh_time_client_evt_t *event)
{
    SWITCH_DEBUG_EVT_SIG(msg->opcode.opcode);

    switch (event->evt_id) {
        case BT_MESH_TIME_CLIENT_EVT_TIME_STATUS: {
                const bt_mesh_time_client_evt_time_status_t *evt = &event->data.time_status;
                LOG_I(mesh_app, "TAISeconds    : %llx\n", evt->tai_seconds);
                LOG_I(mesh_app, "SubSecond     : %02x\n", evt->subsecond);
                LOG_I(mesh_app, "Uncertainty   : %02x\n", evt->uncertainty);
                LOG_I(mesh_app, "TimeAuth      : %02x\n", evt->time_authority);
                LOG_I(mesh_app, "TAI-UTCDelta  : %04x\n", evt->tai_utc_delta);
                LOG_I(mesh_app, "TimeZoneOffset: %02x\n", evt->time_zone_offset);
                break;
            }
        case BT_MESH_TIME_CLIENT_EVT_TIMEZONE_STATUS: {
                const bt_mesh_time_client_evt_timezone_status_t *evt = &event->data.timezone_status;
                LOG_I(mesh_app, "OffsetCur    : %02x\n", evt->offset_current);
                LOG_I(mesh_app, "OffsetNew    : %02x\n", evt->offset_new);
                LOG_I(mesh_app, "TAIZoneChange: %llx\n", evt->tai_of_zone_change);
                break;
            }
        case BT_MESH_TIME_CLIENT_EVT_TAI_UTC_DELTA_STATUS: {
                const bt_mesh_time_client_evt_taiutc_delta_status_t *evt = &event->data.taiutc_status;
                LOG_I(mesh_app, "TAICurrent    : %04x\n", evt->tai_utc_delta_current);
                LOG_I(mesh_app, "Padding1      : %02x\n", evt->padding_1);
                LOG_I(mesh_app, "TAINew        : %04x\n", evt->tai_utc_delta_new);
                LOG_I(mesh_app, "Padding2      : %02x\n", evt->padding_2);
                LOG_I(mesh_app, "TAIDeltaChange: %llx\n", evt->tai_of_delta_change);
                break;
            }
        case BT_MESH_TIME_CLIENT_EVT_TIMEROLE_STATUS: {
                const bt_mesh_time_client_evt_timerole_status_t *evt = &event->data.timerole_status;
                LOG_I(mesh_app, "TimeRole    : %02x\n", evt->time_role);
                break;
            }
        default: {
                bt_mesh_app_util_display_hex_log(&msg->buffer[0], msg->length);
...
static void _scheduler_client_msg_handler(uint16_t model_handle,
                                          const bt_mesh_access_message_rx_t *msg, const bt_mesh_scheduler_client_evt_t *event)
{
    SWITCH_DEBUG_EVT_SIG(msg->opcode.opcode);

    switch (event->evt_id) {
        case BT_MESH_SCHEDULER_CLIENT_EVT_SCHEDULER_STATUS: {
                const bt_mesh_scheduler_client_evt_scheduler_status_t *evt = &event->data.scheduler_status;
                LOG_I(mesh_app, "Schedules    : %04x\n", evt->schedules);
                break;
            }
        case BT_MESH_SCHEDULER_CLIENT_EVT_ACTION_STATUS: {
                const bt_mesh_scheduler_client_evt_action_status_t *evt = &event->data.action_status;
                LOG_I(mesh_app, "Index       : %02x\n", evt->index);
                LOG_I(mesh_app, "Year        : %02x\n", evt->year);
                LOG_I(mesh_app, "Month       : %04x\n", evt->month);
                LOG_I(mesh_app, "Day         : %02x\n", evt->day);
                LOG_I(mesh_app, "Hour        : %02x\n", evt->hour);
                LOG_I(mesh_app, "Minute      : %02x\n", evt->minute);
                LOG_I(mesh_app, "Second      : %02x\n", evt->second);
                LOG_I(mesh_app, "DayOfWeek   : %02x\n", evt->day_of_week);
                LOG_I(mesh_app, "Action      : %02x\n", evt->action);
                LOG_I(mesh_app, "TransTime   : %02x\n", evt->transition_time);
                LOG_I(mesh_app, "SceneNumber : %04x\n", evt->scene_number);
                break;
            }
        default: {
                bt_mesh_app_util_display_hex_log(&msg->buffer[0], msg->length);
...
static void mesh_create_device(void)
{
    ...
    // add configuration server; As a provisioner, configuration server model is optional and can be removed to reduce binary file size.
    bt_mesh_model_add_configuration_server(&g_model_handle_config_server, NULL);

    // add health server; As a provisioner, health server model is optional and can be removed to reduce binary file size.
    ...
    // add scene, time, scheduler client
    bt_mesh_model_add_scene_client(&g_model_handle_scene, element_index, _scene_client_msg_handler, NULL);
    bt_mesh_model_add_time_client(&g_model_handle_time, element_index, _time_client_msg_handler, NULL);
    bt_mesh_model_add_scheduler_client(&g_model_handle_scheduler, element_index, _scheduler_client_msg_handler, NULL);
...
{
    mesh_app_task_event_t event;
    event.event_id = 2;//event id 1 is for timer.
    memcpy(event.prov_data, prov_data, 38);
...
static void _mesh_prov_with_propietary_service(uint8_t *data)
{
    if ((gSwitchState == SWITCH_SETTING_STATE_IDLE) || (gSwitchState >= SWITCH_SETTING_STATE_PROVISIONED)) {
        ...
        if (data[0] == 0 || data[0] == 1) {
            ...
            if (g_start_params == NULL) {
                LOG_E(mesh_app, "memory alloc failed");
                return;
            }
            ...
            // data
            memcpy(g_start_params->data.netkey, &data[17], 16);

            SWITCH_DEBUG_DUMP_PDU("netkey", BT_MESH_NETKEY_SIZE, g_start_params->data.netkey);

            g_start_params->data.netkey_index = data[33];
            g_start_params->data.iv_index = data[34];
            memcpy(&(g_start_params->data.address), data + 35, 2);
            g_start_params->data.flags = data[37];
            LOG_I(mesh_app, "Unicast address[0x%04x] flags[%d]\n", g_start_params->data.address, g_start_params->data.flags);
            ...
            if (data[0] == 0) {
                ...
                // uuid
                memcpy(uuid, &data[1], BT_MESH_UUID_SIZE);

                SWITCH_DEBUG_DUMP_PDU("uuid", BT_MESH_UUID_SIZE, uuid);
                ...
                target_addr.type = g_target_addr.type = data[1];
                memcpy(target_addr.addr, &data[2], 6);
                memcpy(g_target_addr.addr, &data[2], 6);
            ...
            LOG_I(mesh_app, "Byte[0] should be 0 for PB-ADV or 1 for PB-GATT\n");
    } else {
...
void mesh_app_process_event(mesh_app_task_event_t *event)
{
    if (event->event_id == 1) {
        ...
        } else if (timer_id == mesh_config_timer) {
            _configure_callback_post_handler(timer_id);
        } else if (timer_id == mesh_switch_led_timer) {
            _switch_led_timer_cb_post_handler(timer_id);
        } else if (timer_id == mesh_get_remote_info_timer) {
            _get_remote_info_timeout_callback_post_handler(timer_id);
        } else if (timer_id == mesh_node_reset_timer) {
            _node_reset_retry_callback_post_handler(timer_id);
        }
#ifdef MESH_APP_PERFORMANCE_TEST
        else if (timer_id == MESH_LATENCYTEST_TIMER_ID) {
...
#endif /* #ifdef MESH_APP_PERFORMANCE_TEST */
#ifdef MESH_PROPRIETARY_SRV
        else if (timer_id == MESH_ADV_TIMER_ID) {
            _custom_adv_timeout_handler(timer_id);
        }
#endif /* #ifdef MESH_PROPRIETARY_SRV */
        else {
            LOG_E(mesh_app, "unknown event!");
        }
    }
#ifdef MESH_PROPRIETARY_SRV
    if (event->event_id == 2) {
        _mesh_prov_with_propietary_service(event->prov_data);
    }
#endif /* #ifdef MESH_PROPRIETARY_SRV */
}
...
static void mesh_app_task_create(void)
{
...
#endif /* #ifndef PROVISIONER_ENABLE */
...
#ifdef MESH_PROPRIETARY_SRV
static void stop_propietary_service_adv(void)
{
    bt_mesh_app_util_stop_timer(MESH_ADV_TIMER_ID);
}

static void _send_custom_adv_timer_cb(TimerHandle_t handle)
{
    mesh_app_post_timeout_event(handle);
}

static void start_propietary_service_adv(void)
{
    LOG_I(mesh_app, "start_propietary_service_adv");
    bt_mesh_app_util_start_timer_ext("mesh custom adv", &MESH_ADV_TIMER_ID, false, 1000, _send_custom_adv_timer_cb);
}
#endif /* #ifdef MESH_PROPRIETARY_SRV */
bt_status_t mesh_app_bt_event_callback(bt_msg_type_t msg, bt_status_t status, void *buff)
{
    switch (msg) {
        case BT_POWER_ON_CNF: {
                ...
                gMeshDeviceQueue = bt_mesh_os_layer_ds_queue_alloc();
                if (!gMeshDeviceQueue) {
                    LOG_E(mesh_app, "queue alloc err!");
        ...
        case BT_POWER_OFF_CNF: {
                if (gMeshDeviceQueue) {
                    bt_mesh_os_layer_ds_queue_free(gMeshDeviceQueue);
                    gMeshDeviceQueue = NULL;
                }
                break;
            }
        case BT_GAP_LE_ADVERTISING_REPORT_IND: {
                ...
                if (report->event_type == BT_GAP_LE_ADV_REPORT_EVT_TYPE_ADV_IND) {
                    ...
                    if (device != NULL && device->data_len != 0) {
                        ...
                        if (bt_mesh_os_layer_ds_queue_count(gMeshDeviceQueue) > UNPROVISIONED_DEV_REC_MAX) {
                            connectabt_mesh_device_t *oldest_dev = bt_mesh_os_layer_ds_queue_pop(gMeshDeviceQueue);
                            if (oldest_dev->data) {
                                bt_mesh_app_util_free(oldest_dev->data);
                                oldest_dev->data = NULL;
                            }
                            if (oldest_dev) {
                                bt_mesh_os_layer_ds_queue_entry_free(oldest_dev);
                                oldest_dev = NULL;
                            }
                        }
        ...
        case BT_GAP_LE_CONNECT_IND: {
                ...
                bt_gap_le_get_connection_information(connection_ind->connection_handle, &conn_info);

                LOG_I(mesh_app, "BT_GAP_LE_CONNECT_IND\n");

                if (conn_info.role == BT_ROLE_SLAVE) {

                    g_slave_handle = connection_ind->connection_handle;
#ifdef MESH_PROPRIETARY_SRV
                    //stop connectable propietary
                    stop_propietary_service_adv();
#endif /* #ifdef MESH_PROPRIETARY_SRV */
                }
                break;
            }
        case BT_GAP_LE_DISCONNECT_IND: {
                bt_gap_le_disconnect_ind_t *disconn_ind = (bt_gap_le_disconnect_ind_t *)buff;

                if (g_slave_handle == disconn_ind->connection_handle) {
#ifdef MESH_PROPRIETARY_SRV
                    start_propietary_service_adv();
#endif /* #ifdef MESH_PROPRIETARY_SRV */
                    g_slave_handle = 0xFFFF;
                }
                break;
            }
        ...
        case BT_MESH_EVT_BEARER_GATT_STATUS: {
                ...
                if (p->status == BT_MESH_BEARER_GATT_STATUS_CONNECTED) {
                    ...
                    if (g_provision_by_gatt && ((gSwitchState == SWITCH_SETTING_STATE_IDLE) || (gSwitchState >= SWITCH_SETTING_STATE_PROVISIONED))) {
                        ...
                        bt_mesh_provision_invite_provisioning(g_prov_current_uuid, &params);
                        gSwitchState = SWITCH_SETTING_STATE_PROVISIONING;
                    } else {
                        LOG_I(mesh_app, "already in process, state %d\n", gSwitchState);
                    }
...
bt_status_t cmd_config(void *no, uint16_t argc, char **argv)
{
    ...
    if (!strcmp(argv[1], "feature")) {
        ...
        /* binding scene/time/scheduler client models */
        ret = bt_mesh_model_bind_model_app(g_model_handle_scene, g_appkey_index);
        LOG_I(mesh_app, "bind g_model_handle_scene ret: %d\n", ret);
        ret = bt_mesh_model_bind_model_app(g_model_handle_time, g_appkey_index);
        LOG_I(mesh_app, "bind g_model_handle_time ret: %d\n", ret);
        ret = bt_mesh_model_bind_model_app(g_model_handle_scheduler, g_appkey_index);
        LOG_I(mesh_app, "bind g_model_handle_scheduler ret: %d\n", ret);
...
static bt_status_t cmd_prov_init(void *no, uint16_t argc, char **argv)
{
    ...
    if (argc < 1) {
        ...
        LOG_I(mesh_app, "default setting USAGE: prov init <address>\n");
        ...
        LOG_I(mesh_app, "using default settings.");
        bt_mesh_app_util_str2u16HexNum(argv[1], &g_start_params->data.address);

        memcpy(g_start_params->data.netkey, g_def_netkey, sizeof(g_def_netkey));
        g_start_params->data.netkey_index = g_def_netkeyIdx;
        g_start_params->data.iv_index = 0x00000000;
        g_start_params->data.flags = 0x01;
    } else {
    ...
    if (!strcmp(argv[1], "add")) {
        if (!strcmp(argv[2], "net")) {
            if (argc < 5) {
                ...
                LOG_I(mesh_app, "add default netkey.\n");
                bt_mesh_config_add_network_key(g_def_netkey, g_def_netkeyIdx);
                return BT_STATUS_SUCCESS;
            }
            ...
            if (argc < 6) {
                ...
                LOG_I(mesh_app, "add default appkey.\n");
                bt_mesh_config_add_application_key(g_application_key, g_appkey_index, g_def_netkeyIdx);
                return BT_STATUS_SUCCESS;
            }
    ...
    switch (atoi(argv[1])) {
        case 0: { // set
                ...
                if (argc == 9) {
                    ...
                    publication.destination = 0x0001;
                    publication.count_log = 0xff;
                    publication.period_log = 0x06;
                    publication.ttl = 0x05;
                    publication.features = 0x0f;
                    publication.netkey_index = 0x00;
                }
...
static bt_status_t cmd_msg_config_hb_subscribe(void *no, uint16_t argc, char **argv)
{
    ...
    switch (atoi(argv[1])) {
        case 0: { // set
                ...
                bt_mesh_heartbeat_subscription_param_t subscription = {0};
                bt_mesh_app_util_str2u16HexNum(argv[3], &subscription.source);//subs_src is the address where heartbeat msg is from.

                if (argc == 5) {
                    bt_mesh_app_util_str2u16HexNum(argv[4], &subscription.destination);
                    uint8_t tmp = 0;
                    bt_mesh_app_util_str2u8HexNum(argv[4], &tmp);
                    if (tmp > 0x12) {
                        LOG_I(mesh_app, "period log 0x12 ~ 0xFF is prohibited.\n");
                        return BT_STATUS_FAIL;
                    }
                    ...
                    subscription.period_log = 0x11; //period of processing heartbeat messages.0x11 is the maximum value.
                    bt_mesh_app_util_str2u16HexNum(argv[2], &subscription.destination);
                }
...
bt_status_t cmd_iv_test_mode(void *no, uint16_t argc, char **argv)
{
    if (argc < 2)
        return BT_STATUS_FAIL;
...
bt_status_t cmd_gatt_scan(void *no, uint16_t argc, char **argv)
{
    ...
    while (entry != NULL) {
        LOG_I(mesh_app, "addr_type:[%d] addr: %02x%02x%02x%02x%02x%02x , rssi[%d], type: %s, ", entry->addr.type,
...
bt_status_t cmd_gatt_connect(void *no, uint16_t argc, char **argv)
{
    if (argc == 4) {
        ...
        target_addr.type = g_target_addr.type = atoi(argv[1]);
        bt_mesh_app_util_str2u8HexAddrArray(argv[2], target_addr.addr);
        bt_mesh_app_util_str2u8HexAddrArray(argv[2], g_target_addr.addr);

        connectabt_mesh_device_t *entry = (connectabt_mesh_device_t *)bt_mesh_os_layer_ds_queue_first(gMeshDeviceQueue);
        while (entry != NULL) {
            if (memcmp(&entry->addr, &target_addr, sizeof(bt_addr_t)) == 0) {
                if (entry->type == BT_MESH_GATT_SERVICE_PROVISION) {
                    memcpy(g_prov_current_uuid, entry->data, entry->data_len);
                }
                break;
            }
            entry = (connectabt_mesh_device_t *)bt_mesh_os_layer_ds_queue_next(gMeshDeviceQueue, entry);
        ...
        if (entry == NULL) {
            LOG_W(mesh_app, "No devie record found!");
            return BT_STATUS_SUCCESS;
        }
...
bt_status_t cmd_dump_ext(void *no, uint16_t argc, char **argv)
{
    if (argc >= 2) {
        if (!strcmp(argv[1], "remote_node_info")) {
            if (argc < 4) {
                LOG_I(mesh_app, "USAGE: dump remote_node_info <remote_addr> <lpn_addr>");
                return BT_STATUS_FAIL;
            }
            ...
            if (argc < 3) {
                LOG_I(mesh_app, "USAGE: dump set_node_info <remote_addr>");
                return BT_STATUS_FAIL;
            }
            uint16_t remote_addr;
            bt_mesh_app_util_str2u16HexNum(argv[2], &remote_addr);
...
#endif /* #ifdef APP_BT_MESH_OTA_ENABLE */
static bt_status_t cmd_scene_get(void *no, uint16_t argc, char **argv)
{
    if (argc < 3) {
        printf("USAGE: scene get <dst_addr> <appidx>\n");
        return BT_STATUS_FAIL;
    }
    bt_mesh_status_t ret = 0;
    bt_mesh_access_message_tx_meta_t tx_meta;
    bt_mesh_app_util_str2u16HexNum(argv[1], &tx_meta.dst_addr.value);
    bt_mesh_app_util_str2u16HexNum(argv[2], &tx_meta.appkey_index);
    tx_meta.ttl = bt_mesh_config_get_default_ttl();

    ret = bt_mesh_model_scene_client_get(g_model_handle_scene, &tx_meta);

    printf("ret = %d\n", ret);
    return BT_STATUS_SUCCESS;
}

static bt_status_t cmd_scene_store(void *no, uint16_t argc, char **argv)
{
    if (argc < 5) {
        printf("USAGE: scene store <dst_addr> <appidx> <scene_number> <reliable>\n");
        return BT_STATUS_FAIL;
    }
    ...
    bt_mesh_app_util_str2u16HexNum(argv[3], &scene_number);
    ret = bt_mesh_model_scene_client_store(g_model_handle_scene, &tx_meta, scene_number, atoi(argv[4]) != 0);

    printf("ret = %d\n", ret);
    return BT_STATUS_SUCCESS;
}

static bt_status_t cmd_scene_recall(void *no, uint16_t argc, char **argv)
{
    if (argc < 5) {
        printf("USAGE: scene recall <dst_addr> <appidx> <buffer> <reliable>\n");
        return BT_STATUS_FAIL;
    }
    ...
    uint16_t buf_len;
    uint8_t buf[40] = {0};
    buf_len = bt_mesh_app_util_str2u8HexNumArray(argv[3], buf);
    bt_mesh_app_util_display_hex_log(buf, buf_len);

    ret = bt_mesh_model_scene_client_recall(g_model_handle_scene, &tx_meta, buf, buf_len, atoi(argv[4]) != 0);

    printf("ret = %d\n", ret);
    return BT_STATUS_SUCCESS;
}

static bt_status_t cmd_scene_delete(void *no, uint16_t argc, char **argv)
{
    if (argc < 5) {
        printf("USAGE: scene delete <dst_addr> <appidx> <scene_number> <reliable>\n");
        return BT_STATUS_FAIL;
    }
    ...
    uint16_t scene_number;
    bt_mesh_app_util_str2u16HexNum(argv[3], &scene_number);
    ret = bt_mesh_model_scene_client_delete(g_model_handle_scene, &tx_meta, scene_number, atoi(argv[4]) != 0);

    printf("ret = %d\n", ret);
    return BT_STATUS_SUCCESS;
}

static bt_status_t cmd_scene_getr(void *no, uint16_t argc, char **argv)
{
    if (argc < 3) {
        printf("USAGE: scene getr <dst_addr> <appidx>\n");
        return BT_STATUS_FAIL;
    }
    ...
    ret = bt_mesh_model_scene_client_get_register(g_model_handle_scene, &tx_meta);

    printf("ret = %d\n", ret);
    return BT_STATUS_SUCCESS;
}

static bt_status_t cmd_time_time(void *no, uint16_t argc, char **argv)
{
    if (argc < 4) {
        printf("USAGE: time time <action> <dst_addr> <app_idx> [<tai_sec> <subsec> <uncertainty> <time_auth> <tai_utc_delta> <time_zone_offset>]\n");
        printf("<action> 0: get, 1: set\n");
        return BT_STATUS_FAIL;
    }

    bt_mesh_status_t ret = 0;
    bt_mesh_access_message_tx_meta_t tx_meta;
    bt_mesh_app_util_str2u16HexNum(argv[2], &tx_meta.dst_addr.value);
    bt_mesh_app_util_str2u16HexNum(argv[3], &tx_meta.appkey_index);
    tx_meta.ttl = bt_mesh_config_get_default_ttl();

    switch (atoi(argv[1])) {
        case 0: {
                ret = bt_mesh_model_time_client_get_time(g_model_handle_time, &tx_meta);
                break;
            }
        case 1: {
                bt_mesh_time_client_evt_time_status_t param = {0};
                uint8_t tmp8 = 0;
                uint16_t tmp16 = 0;

                param.tai_seconds = 0x100000000;
                param.rfu = 0;
                bt_mesh_app_util_str2u8HexNum(argv[5], &tmp8);
                param.subsecond = tmp8;
                bt_mesh_app_util_str2u8HexNum(argv[6], &tmp8);
                param.uncertainty = tmp8;
                bt_mesh_app_util_str2u8HexNum(argv[7], &tmp8);
                param.time_authority = tmp8;
                bt_mesh_app_util_str2u16HexNum(argv[8], &tmp16);
                param.tai_utc_delta = tmp16;
                bt_mesh_app_util_str2u8HexNum(argv[9], &tmp8);
                param.time_zone_offset = tmp8;
                ret = bt_mesh_model_time_client_set_time(g_model_handle_time, &tx_meta, &param);
                break;
            }
    }
...
static bt_status_t cmd_time_zone(void *no, uint16_t argc, char **argv)
{
    if (argc < 4) {
        printf("USAGE: time zone <action>\n");
        return BT_STATUS_FAIL;
    }
    ...
    switch (atoi(argv[1])) {
        case 0: {
                ret = bt_mesh_model_time_client_get_time_zone(g_model_handle_time, &tx_meta);
                break;
            }
        case 1: {
                bt_mesh_time_client_timezone_set_t param = {0};
                uint8_t tmp8 = 0;

                bt_mesh_app_util_str2u8HexNum(argv[4], &tmp8);
                param.time_zone_offset_new = tmp8;
                param.tai_of_zone_change = 0x123456789;
                ret = bt_mesh_model_time_client_set_time_zone(g_model_handle_time, &tx_meta, &param);
                break;
            }
    }
...
static bt_status_t cmd_time_tai(void *no, uint16_t argc, char **argv)
{
    if (argc < 4) {
        printf("USAGE: time tai <action>\n");
        return BT_STATUS_FAIL;
    }
    ...
    switch (atoi(argv[1])) {
        case 0: {
                ret = bt_mesh_model_time_client_get_taiutc_delta(g_model_handle_time, &tx_meta);
                break;
            }
        case 1: {
                bt_mesh_time_client_evt_taiutc_delta_set_t param = {0};
                uint16_t tmp16 = 0;

                bt_mesh_app_util_str2u16HexNum(argv[4], &tmp16);
                param.tai_utc_delta_new = tmp16;
                param.tai_of_delta_change = 0x123456789;
                ret = bt_mesh_model_time_client_set_taiutc_delta(g_model_handle_time, &tx_meta, &param);
                break;
            }
    }
...
static bt_status_t cmd_time_role(void *no, uint16_t argc, char **argv)
{
    if (argc < 4) {
        printf("USAGE: time role <action>\n");
        return BT_STATUS_FAIL;
    }
    ...
    switch (atoi(argv[1])) {
        case 0: {
                ret = bt_mesh_model_time_client_get_time_role(g_model_handle_time, &tx_meta);
                break;
            }
        case 1: {
                bt_mesh_time_client_evt_timerole_status_t param = {0};
                uint8_t tmp8 = 0;

                bt_mesh_app_util_str2u8HexNum(argv[4], &tmp8);
                param.time_role = tmp8;
                ret = bt_mesh_model_time_client_set_time_role(g_model_handle_time, &tx_meta, &param);
                break;
            }
    }
...
static bt_status_t cmd_sched(void *no, uint16_t argc, char **argv)
{
    if (argc < 4) {
        printf("USAGE: sched <action> <dst_addr> <appidx>\n");
        printf("<action> 0: get, 1: action set, 2: action set_unack, 3: action get\n");
        return BT_STATUS_FAIL;
    }
    ...
    switch (atoi(argv[1])) {
        case 0: {
                bt_mesh_model_scheduler_client_get(g_model_handle_scheduler, &tx_meta);
                break;
            }
        case 1:
        case 2: {
                bt_mesh_scheduler_client_evt_action_status_t param = {0};
                param.index = 0xF;
                param.year = 0x11;
                param.month = 0x0004;
                param.day = 0x18;
                ...
                param.minute = 0x3D;
                param.second = 0x3C;
                param.day_of_week = 0x10;
                param.action = 0x01;
                param.transition_time = 0x3E;
                param.scene_number = 0x1234;
                bt_mesh_model_scheduler_client_set_action(g_model_handle_scheduler, &tx_meta, &param, atoi(argv[1]) == 1);
                break;
            }
        case 3: {
                uint8_t index;
                bt_mesh_app_util_str2u8HexNum(argv[4], &index);
                bt_mesh_model_scheduler_client_get_action(g_model_handle_scheduler, &tx_meta, index);
                break;
            }
    }
...
const mesh_app_callback_table_t cmd_tbl_scene[] = {
    {"get", cmd_scene_get},
    {"store", cmd_scene_store},
    {"recall", cmd_scene_recall},
    {"delete", cmd_scene_delete},
    {"getr", cmd_scene_getr},
    {NULL, NULL}
};

bt_status_t cmd_scene(void *no, uint16_t argc, char **argv)
{
    uint16_t i;
    for (i = 0; i < sizeof(cmd_tbl_scene) / sizeof(mesh_app_callback_table_t); i++) {
        if (!strcmp(cmd_tbl_scene[i].name, argv[1])) {
            return cmd_tbl_scene[i].io_callback(no, argc - 1, argv + 1);
        }
    }
    printf("scene sub-command not found\n");
    return BT_STATUS_FAIL;

}

const mesh_app_callback_table_t cmd_tbl_time[] = {
    {"time", cmd_time_time},
    {"zone", cmd_time_zone},
    {"tai", cmd_time_tai},
    {"role", cmd_time_role},
...
bt_status_t cmd_time(void *no, uint16_t argc, char **argv)
{
    uint16_t i;
    for (i = 0; i < sizeof(cmd_tbl_time) / sizeof(mesh_app_callback_table_t); i++) {
        if (!strcmp(cmd_tbl_time[i].name, argv[1])) {
            return cmd_tbl_time[i].io_callback(no, argc - 1, argv + 1);
        }
    }
    printf("time sub-command not found\n");
    return BT_STATUS_FAIL;

}

//This func is referenced in bt_setting.c to set mesh log level when bt is initializing.
...
#endif /* #ifndef __MESH_APP_SWITCH_C__ */
