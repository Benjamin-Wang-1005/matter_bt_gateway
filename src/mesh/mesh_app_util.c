...
static bool bt_app_mesh_enabled = false;

/******************************************************************************/
/* extern function                                                          */
/******************************************************************************/

extern bt_mesh_status_t meshif_enable_mesh_filter(meshif_filter_params_t *filter_params);
extern bt_mesh_status_t meshif_disable_mesh_filter(void);

/*****************************************************************************/
/* main                                                                      */
/*****************************************************************************/

void mesh_app_util_log_off(void)
{
...
void mesh_app_util_copy_str_to_addr(uint8_t *addr, const char *str)
{
    ...
    for (i = 0; i < 6; i++) {
        ret = sscanf(str + using_hex_sign + i * (2 + using_long_format), "%02x", &value);
        if (ret < 0) {
            LOG_I(mesh_app, "%s, sscanf fail, ret = %d", __func__, ret);
            return;
        }
...
{
    if (*timer_id == NULL) {
        *timer_id = xTimerCreate((const char *const)name, 0xffff, auto_reload, NULL, pxCallbackFunction);
    }
}
...
void bt_mesh_app_util_deinit_timer(TimerHandle_t timer_id)
{
    ...
    if (timer_id != NULL) {
        ret = xTimerDelete(timer_id, 0);
        if (ret != pdPASS) {
            LOG_I(mesh_app, "[MESH]bt_mesh_app_util_deinit_timer fail 0x%x", ret);
            configASSERT(0);
        }
    }
}
...
void bt_mesh_app_util_stop_timer(TimerHandle_t timer_id)
{
    ...
    if ((timer_id != NULL) && (bt_mesh_app_util_is_timer_active(timer_id) == 1)) {
        ...
        if (ret != pdPASS) {
            LOG_I(mesh_app, "[MESH]bt_mesh_app_util_stop_timer fail 0x%x", ret);
            configASSERT(0);
        }
    }
}
...
void bt_mesh_app_util_start_timer(TimerHandle_t timer_id, uint32_t ms)
{
    ...
    if (ret != pdPASS) {
        LOG_I(mesh_app, "[MESH]bt_mesh_app_util_start_timer fail 0x%x", ret);
        configASSERT(0);
    }
}
...
void bt_mesh_app_util_start_timer_ext(char *name, TimerHandle_t *timer_id, bool is_repeat, uint32_t ms, TimerCallbackFunction_t pxCallbackFunction)
{
    ...
    ret = xTimerChangePeriod(*timer_id, time_length, 0);
    if (ret != pdPASS) {
        LOG_I(mesh_app, "[MESH]bt_mesh_app_util_start_timer_ext fail 0x%x", ret);
        configASSERT(0);
    }
}
...
{
    uint32_t temp;
    bool ret = bt_mesh_app_util_str2u32HexNum(src, &temp);
    if (ret) {
        *dst = (uint8_t)temp;
    }
    return ret;
...
bool bt_mesh_app_util_str2u32HexNum(const char *src, uint32_t *dst)
{
    ...
    if (src == NULL || dst == NULL) {
        LOG_E(mesh_app, "%s, param dst is NULL", __func__);
        return false;
    }
    *dst = (uint32_t)strtoul(src, &end, 10);
    if (*dst == ULONG_MAX) {
        LOG_E(mesh_app, "%s, strtoul base=10 fail, ERANGE", __func__);
        return false;
    }
    if (*end) {
        *dst = (uint32_t)strtoul(src, &end, 16);
        if (*dst == ULONG_MAX) {
            LOG_E(mesh_app, "%s, strtoul base=16 fail, ERANGE", __func__);
            return false;
        }
...
{
    LOG_HEXDUMP_I(mesh_app, " ", buffer, len);
...
void mesh_app_util_copy_str_to_2byte(uint16_t *des, const char *str, uint32_t len)
{
    ...
    for (i = 0; i < len; i++) {
        ret = sscanf(str + using_hex_sign + i * (2 + using_long_format), "%04x", &value);
        if (ret < 0) {
            LOG_E(mesh_app, "%s, sscanf fail, ret = %d", __func__, ret);
            return;
        }
...
{
    LOG_HEXDUMP_I(mesh_app, "%s[%d] :", pdu, pdu_len, pdu_name, pdu_len);
...
{
    LOG_HEXDUMP_I(mesh_app, "%s[%d] :", pdu, pdu_len * 2, pdu_name, pdu_len * 2);
...
void bt_mesh_app_util_check_device_uuid(uint8_t *device_uuid)
{
    ...
    if (NVDM_STATUS_OK == status && size == BT_MESH_UUID_SIZE) {
        ...
        vTaskDelay(3000 / portTICK_PERIOD_MS); //tt test
...
{
    LOG_I(mesh_app, "config wifi_service_time %d ms\n", time);
    uint8_t wifi_time = time * 1000 / 625;
...
extern void porting_mesh_bearer_adv_dump(void);
extern void mesh_friend_dump(void);
extern void mesh_proxy_dump(void);
...
bt_status_t cmd_dump(void *no, uint16_t argc, char **argv)
{
    ...
    if (!strcmp(argv[1], "net")) {
        ...
        if (data == NULL) {
            LOG_E(mesh_app, "get composition data fail");
        ...
        if (data == NULL) {
            LOG_E(mesh_app, "get network retransmit fail");
            return BT_STATUS_FAIL;
        }
        ...
        if (data == NULL) {
            LOG_E(mesh_app, "get relay retransmit fail");
            return BT_STATUS_FAIL;
        }
...
extern bt_mesh_status_t porting_mesh_bearer_adv_enable(void);
extern bt_mesh_status_t porting_mesh_bearer_adv_disable(void);
bt_status_t cmd_bearer(void *no, uint16_t argc, char **argv)
{
    if (argc < 3) {
        LOG_I(mesh_app, "USAGE: bearer adv <adv_period> <adv_interval> <retry>\n");
        LOG_I(mesh_app, "USAGE: bearer scan <scan_period> <scan_interval> <scan_window>\n");
        LOG_I(mesh_app, "USAGE: bearer filter <mode 0:disable, 1:enable>\n");
        return BT_STATUS_SUCCESS;
    }

    porting_mesh_bearer_adv_disable();
    vTaskDelay(50 / portTICK_PERIOD_MS); //Wait a while to make sure disable can be completed
    if (!strcmp(argv[1], "adv")) {
        bt_mesh_bearer_adv_params_t adv_params = {
            .adv_period = 40,
            .min_interval = 0x8,   /* 5ms */
            .max_interval = 0x8,   /* 5ms */
            .resend = 1
        };
        if (argc >= 3) {
            bt_mesh_app_util_str2u32HexNum(argv[2], &adv_params.adv_period);
        }
        if (argc >= 4) {
            bt_mesh_app_util_str2u16HexNum(argv[3], &adv_params.min_interval);
            adv_params.max_interval = adv_params.min_interval;
        }
        if (argc >= 5) {
            bt_mesh_app_util_str2u8HexNum(argv[4], &adv_params.resend);
        }
        LOG_I(mesh_app, "cli adv, period =%d, interval =%d, resend =%d \n",
              adv_params.adv_period, adv_params.min_interval, adv_params.resend);
        bt_mesh_bearer_adv_set_params(&adv_params, NULL);
        ...
        bt_mesh_app_util_str2u32HexNum(argv[2], &scan_params.scan_period);
        if (argc >= 5) {
            bt_mesh_app_util_str2u16HexNum(argv[3], &scan_params.scan_interval);
            bt_mesh_app_util_str2u16HexNum(argv[4], &scan_params.scan_window);
        }
        LOG_I(mesh_app, "cli scan, period =%d, interval =%d, window =%d \n",
              scan_params.scan_period, scan_params.scan_interval, scan_params.scan_window);
        bt_mesh_bearer_adv_set_params(NULL, &scan_params);
    }
#if 0 //disable this cli in qfn due to lake of code size (This feature need +1744bytes)
    else if (!strcmp(argv[1], "filter")) {
        uint8_t mode = (uint8_t)strtoul(argv[2], NULL, 10);
        bt_mesh_app_util_filter_mode(mode);
    }
#endif
    porting_mesh_bearer_adv_enable();
    return BT_STATUS_SUCCESS;
}

/* MTK TCI command to set mesh mode for controller */
void bt_mesh_app_util_set_ctr_mesh_mode(ble_ctr_mesh_mode_t mode)
{
    LOG_I(mesh_app, "set ctr mesh mode = 0x%x", mode);
    uint16_t cmd_code = 0xFD80;
    uint8_t ctr_mode = (uint8_t) mode;

    bt_hci_send_vendor_cmd(cmd_code, &ctr_mode, 1);

    return;
}

void bt_mesh_app_util_filter_mode(mesh_fiter_mode_t mode)
{
    meshif_filter_params_t filter;
    bt_mesh_status_t status;

    LOG_I(mesh_app, "set mesh filter mode = %d", mode);

    if (MESH_FILTER_DISABLE_ALL == mode) {
        status = meshif_disable_mesh_filter();
        if (status != BT_MESH_SUCCESS)
            LOG_E(mesh_app, "set mesh filter fail (ret = %d)", status);
        return;
    }
    ...
    switch (mode) {
        case MESH_FILTER_ENABLE_ALL:
            filter.pb_adv = true;
            filter.mesh_message = true;
            filter.unprov_beacon = true;
            filter.secure_beacon = true;
            break;
        /* Not verified
        case MESH_FILTER_UNPROV_ONLY:
            filter.pb_adv = true;
            filter.unprov_beacon = true;
            break;
        case MESH_FILTER_PROV_ONLY:
            filter.mesh_message = true;
            filter.secure_beacon = true;
            break;
        */
    ...
    status = meshif_enable_mesh_filter(&filter);
    if (status != BT_MESH_SUCCESS) {
        LOG_E(mesh_app, "set mesh filter fail (ret = %d)", status);
    }
...
{
    return bt_app_mesh_enabled;
}

void bt_app_mesh_set_enable(bool is_enabled)
{
    bt_app_mesh_enabled = is_enabled;
}
