...
static bt_status_t cmd_test(void *no, uint16_t argc, char **argv)
{
    if (argc < 2) {
        return BT_STATUS_FAIL;
    }
    ...
    if (!strcmp(argv[1], "enable")) {
        LOG_I(mesh_app, "manually enable mesh...");
        if (!mesh_inited) {
            LOG_E(mesh_app, "init mesh first!");
            return BT_STATUS_FAIL;
        }
        ...
        LOG_I(mesh_app, "manually disable mesh...");
        if (!bt_app_mesh_is_enabled())
            return BT_STATUS_SUCCESS;
        bt_mesh_disable();
        bt_app_mesh_set_enable(false);
    } else {
        LOG_W(mesh_app, "unknown mesh test cmd!");
        return BT_STATUS_FAIL;
    }
