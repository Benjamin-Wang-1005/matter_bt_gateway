...
static void led2_set_ctl(uint16_t temperature, int16_t delta_uv, uint16_t ligntness)
{
...
static void _hsl_device_timer_handler(TimerHandle_t idx)
{
    ...
    if (MESH_GENERIC_LEVEL_DELAY_TIMER_ID_HSL == idx) {
        ...
        led2_set_level(gHSL_server->lightness_server.level_server.present_level);
...
static void _send_generic_on_off_status(lighting_lightness_server_t *p_lightness_server,
                                        uint16_t model_handle, const bt_mesh_access_message_rx_t *msg, const void *arg)
{
    uint8_t *reply;
    uint8_t length = 0;
...
static void _send_generic_level_status(uint16_t model_handle, const bt_mesh_access_message_rx_t *msg, const void *arg)
{
    uint8_t *reply;
    uint8_t length = 0;
    ...
    } else if (element_idx == gHSL_server->element_index) {
        p_level_server = &gHSL_server->lightness_server.level_server;
    } else if (element_idx == gCTL_temperature_server->element_index) {
...
static void _send_lighting_lightness_status(lighting_lightness_server_t *p_lightness_server,
                                            uint16_t model_handle, const bt_mesh_access_message_rx_t *msg, const void *arg)
{
    uint8_t *reply;
    uint8_t length = 0;
...
static void _send_lighting_lightness_linear_status(lighting_lightness_server_t *p_lightness_server,
                                                   uint16_t model_handle, const bt_mesh_access_message_rx_t *msg, const void *arg)
{
    uint8_t *reply;
    uint8_t length = 0;
...
static void _send_lighting_ctl_status(uint16_t model_handle, const bt_mesh_access_message_rx_t *msg, const void *arg)
{
    uint8_t *reply;
    uint8_t length = 0;
...
static void _send_lighting_ctl_temperature_status(uint16_t model_handle, const bt_mesh_access_message_rx_t *msg, const void *arg)
{
    uint8_t *reply;
    uint8_t length = 0;
...
static void _send_lighting_hsl_status(uint16_t model_handle, const bt_mesh_access_message_rx_t *msg, const void *arg)
{
    uint8_t *reply;
    uint8_t length = 0;
...
static void _send_lighting_hsl_hue_status(uint16_t model_handle, const bt_mesh_access_message_rx_t *msg, const void *arg)
{
    uint8_t *reply;
    uint8_t length = 0;
...
static void _send_lighting_hsl_saturation_status(uint16_t model_handle, const bt_mesh_access_message_rx_t *msg, const void *arg)
{
    uint8_t *reply;
    uint8_t length = 0;
                                           ...
                                           uint16_t model_handle, const bt_mesh_access_message_rx_t *msg, const void *arg, bool is_unacknowledged, vendordevice_led_type_t type)
{
    ...
    if (msg->length > 3) {
        ...
        lightness_server->level_server.delay = msg->buffer[4];
    } else {
                                           ...
                                           uint16_t model_handle, const bt_mesh_access_message_rx_t *msg, const void *arg, bool is_unacknowledged, vendordevice_led_type_t type)
{
    ...
    if (msg->length > 5) {
        ...
        lightness_server->level_server.delay = msg->buffer[6];
    } else {
        lightness_server->level_server.transition_time = lightness_server->dtt_server->default_transition_time;
    }
    ...
    const void *arg, bool is_unacknowledged)
{
    ...
    if (msg->length > 5) {
        ...
        gHSL_hue_server->gLevel_server.delay = msg->buffer[6];
    } else {
        gHSL_hue_server->gLevel_server.transition_time = gHSL_server->lightness_server.dtt_server->default_transition_time;
    }
    ...
    const void *arg, bool is_unacknowledged)
{
    ...
    if (msg->length > 5) {
        gHSL_saturation_server->gLevel_server.transition_time = (msg->buffer[5] & 0x3F) ?
                                                                msg->buffer[5] : gHSL_server->lightness_server.dtt_server->default_transition_time;
        gHSL_saturation_server->gLevel_server.delay = msg->buffer[6];
    } else {
        gHSL_saturation_server->gLevel_server.transition_time = gHSL_server->lightness_server.dtt_server->default_transition_time;
    }
                                          ...
                                          uint16_t model_handle, const bt_mesh_access_message_rx_t *msg, const void *arg, bool is_unacknowledged, vendordevice_led_type_t type)
{
    ...
    if (lightness_server->level_server.TID == msg->buffer[2]) {
        if (!is_unacknowledged)
            _send_generic_level_status(model_handle, msg, arg);
        return;
    }
    ...
    if (msg->length > 3) {
        lightness_server->level_server.transition_time = msg->buffer[3];
        lightness_server->level_server.delay = msg->buffer[4];
    } else {
        lightness_server->level_server.transition_time = lightness_server->dtt_server->default_transition_time;
    }
                                          ...
                                          uint16_t model_handle, const bt_mesh_access_message_rx_t *msg, const void *arg, bool is_unacknowledged, vendordevice_led_type_t type)
{
    ...
    if (lightness_server->TID == msg->buffer[2]) {
        if (!is_unacknowledged)
            _send_lighting_lightness_status(lightness_server, model_handle, msg, arg);
        return;
    }
    ...
    if (msg->length > 3) {
        lightness_server->transition_time = msg->buffer[3];
    } else {
                                          ...
                                          uint16_t model_handle, const bt_mesh_access_message_rx_t *msg, const void *arg, bool is_unacknowledged, vendordevice_led_type_t type)
{
    ...
    if (lightness_server->TID == msg->buffer[2]) {
        if (!is_unacknowledged)
            _send_lighting_lightness_linear_status(lightness_server, model_handle, msg, arg);
                                       ...
                                       const bt_mesh_access_message_rx_t *msg, const void *arg, bool is_unacknowledged)
{
    ...
    if (gCTL_server->TID == msg->buffer[6]) {
        if (!is_unacknowledged)
            _send_lighting_ctl_status(model_handle, msg, arg);
    ...
    if (msg->length > 7) {
        gCTL_server->transition_time = msg->buffer[7];
    } else {
    ...
    if (reply != NULL) {
        ...
        VD_DEBUG_DUMP_PDU("ctl_set_status load:", length, reply);
                                                   ...
                                                   const bt_mesh_access_message_rx_t *msg, const void *arg, bool is_unacknowledged)
{
    ...
    if (gCTL_temperature_server->TID == msg->buffer[4]) {
        if (!is_unacknowledged)
            _send_lighting_ctl_temperature_status(model_handle, msg, arg);
        return;
    }
    ...
    if (msg->length > 5) {
        gCTL_temperature_server->transition_time = msg->buffer[5];
    } else {
                                       ...
                                       const bt_mesh_access_message_rx_t *msg, const void *arg, bool is_unacknowledged)
{
    ...
    if (gHSL_server->TID == msg->buffer[6]) {
        if (!is_unacknowledged)
            _send_lighting_hsl_status(model_handle, msg, arg);
        return;
    }
    ...
    if (msg->length > 7) {
        gHSL_server->transition_time = msg->buffer[7];
    } else {
                                           ...
                                           const bt_mesh_access_message_rx_t *msg, const void *arg, bool is_unacknowledged)
{
    ...
    if (gHSL_hue_server->TID == msg->buffer[2]) {
        if (!is_unacknowledged)
            _send_lighting_hsl_hue_status(model_handle, msg, arg);
        return;
    }
    ...
    if (msg->length > 3) {
        gHSL_hue_server->transition_time = msg->buffer[3];
    } else {
                                                  ...
                                                  const bt_mesh_access_message_rx_t *msg, const void *arg, bool is_unacknowledged)
{
    ...
    if (gHSL_saturation_server->TID == msg->buffer[2]) {
        if (!is_unacknowledged)
            _send_lighting_hsl_saturation_status(model_handle, msg, arg);
        return;
    }
    ...
    if (msg->length > 3) {
        gHSL_saturation_server->transition_time = msg->buffer[3];
    } else {
...
void light_hsl_server_msg_handler(uint16_t model_handle, const bt_mesh_access_message_rx_t *msg, const void *arg)
{
    ...
    switch (msg->opcode.opcode) {
        case BT_MESH_MODEL_GENERIC_ONOFF_GET: {
                ...
                _send_generic_on_off_status(p_lightness_server, model_handle, msg, arg);
                break;
            }
        ...
        case BT_MESH_MODEL_GENERIC_ONOFF_SET_UNACKNOWLEDGED: {
                ...
                VD_DEBUG_EVT_PRINTF("GenericOnOffSetUnack", 2);
        ...
        case BT_MESH_MODEL_GENERIC_LEVEL_SET_UNACKNOWLEDGED: {
                VD_DEBUG_EVT_PRINTF("GenericLevelSetUnack", 2);
        ...
        case BT_MESH_MODEL_GENERIC_DELTA_SET_UNACKNOWLEDGED: {
                VD_DEBUG_EVT_PRINTF("GenericDeltaSetUnack", 2);
        ...
        case BT_MESH_MODEL_GENERIC_MOVE_SET_UNACKNOWLEDGED: {
                VD_DEBUG_EVT_PRINTF("GenericMoveSetUnack", 2);
        ...
        case BT_MESH_MODEL_GENERIC_DEFAULT_TRANSITION_TIME_SET_UNACKNOWLEDGED: {
                ...
                p_lightness_server->dtt_server->default_transition_time = msg->buffer[0];
                VD_DEBUG_EVT_PRINTF("GenericDefaultTransitionTimeSetUnack", 2);
                LOG_I(mesh_app, "time  : %x\n", p_lightness_server->dtt_server->default_transition_time);
                break;
            }
        ...
        case BT_MESH_MODEL_GENERIC_ONPOWERUP_SET_UNACKNOWLEDGED: {
                VD_DEBUG_EVT_PRINTF("GenericOnPowerupSetUnack", 2);
        ...
        case BT_MESH_MODEL_LIGHT_LIGHTNESS_SET_UNACKNOWLEDGED: {
                VD_DEBUG_EVT_PRINTF("LightLightnessSetUnack", 2);
        ...
        case BT_MESH_MODEL_LIGHT_LIGHTNESS_GET: {
                ...
                _send_lighting_lightness_status(p_lightness_server, model_handle, msg, arg);
        ...
        case BT_MESH_MODEL_LIGHT_LIGHTNESS_LINEAR_SET_UNACKNOWLEDGED: {
                VD_DEBUG_EVT_PRINTF("LightLightnessLinearSetUnack", 2);
        ...
        case BT_MESH_MODEL_LIGHT_LIGHTNESS_LINEAR_GET: {
                ...
                _send_lighting_lightness_linear_status(p_lightness_server, model_handle, msg, arg);
        ...
        case BT_MESH_MODEL_LIGHT_LIGHTNESS_DEFAULT_SET_UNACKNOWLEDGED: {
                VD_DEBUG_EVT_PRINTF("LightLightnessDefaultSetUnack", 2);
        ...
        case BT_MESH_MODEL_LIGHT_LIGHTNESS_RANGE_SET_UNACKNOWLEDGED: {
                VD_DEBUG_EVT_PRINTF("LightLightnessRangeSetUnack", 2);
        ...
        case BT_MESH_MODEL_LIGHT_HSL_SET_UNACKNOWLEDGED: {
                VD_DEBUG_EVT_PRINTF("LightHSLSetUnack", 2);
        ...
        case BT_MESH_MODEL_LIGHT_HSL_SATURATION_SET_UNACKNOWLEDGED: {
                VD_DEBUG_EVT_PRINTF("LightHSLSaturationSetUnack", 2);
        ...
        case BT_MESH_MODEL_LIGHT_HSL_DEFAULT_SET_UNACKNOWLEDGED: {
                VD_DEBUG_EVT_PRINTF("LightHSLDefaultSetUnack", 2);
        ...
        case BT_MESH_MODEL_LIGHT_HSL_RANGE_SET_UNACKNOWLEDGED: {
                uint16_t min, max;
                VD_DEBUG_EVT_PRINTF("LightHSLRangeSetUnack", 2);
...
void light_ctl_server_msg_handler(uint16_t model_handle, const bt_mesh_access_message_rx_t *msg, const void *arg)
{
    ...
    switch (msg->opcode.opcode) {
        ...
        case BT_MESH_MODEL_LIGHT_CTL_SET_UNACKNOWLEDGED: {
                VD_DEBUG_EVT_PRINTF("LightCTLSetUnack", 2);
        ...
        case BT_MESH_MODEL_LIGHT_CTL_TEMPERATURE_SET_UNACKNOWLEDGED: {
                VD_DEBUG_EVT_PRINTF("LightCTLTemperatureSetUnack", 2);
        ...
        case BT_MESH_MODEL_LIGHT_CTL_TEMPERATURE_RANGE_SET_UNACKNOWLEDGED: {
                VD_DEBUG_EVT_PRINTF("LightCTLTemperatureRangeSetUnack", 2);
        ...
        case BT_MESH_MODEL_LIGHT_CTL_DEFAULT_SET_UNACKNOWLEDGED: {
                uint16_t default_temperature;
                VD_DEBUG_EVT_PRINTF("LightCTLDefaultSetUnack", 2);
